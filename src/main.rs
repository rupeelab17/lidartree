...
fn load_geotiff(path: &str) -> Raster {
    ...
    let res_x = 1.0;
    let res_y = 1.0;
    let origin_x = 379818.38063099835;
    let origin_y = 6573607.806912625;
    ...
    let band_data: Vec<f64> = if n_bands > 1 {
        ...
        (0..total_pixels).map(|px| raw[px * n_bands + best_band]).collect()
    } else {
        raw
    };
...
fn main() {
    ...
    while i < args.len() {
        match args[i].as_str() {
            "--hmin"       => { hmin = args[i+1].parse().unwrap(); i += 2; }
            "--dmin"       => { dmin = args[i+1].parse().unwrap(); i += 2; }
            "--dprop"      => { dprop = args[i+1].parse().unwrap(); i += 2; }
            "--sigma"      => { sigma = args[i+1].parse().unwrap(); i += 2; }
            "--median"     => { median_size = args[i+1].parse().unwrap(); i += 2; }
            "--crown-prop" => { crown_prop = Some(args[i+1].parse().unwrap()); i += 2; }
            "--crown"      => { compute_crown = true; i += 1; }
            "--output"     => { output_csv = args[i+1].clone(); i += 2; }
            other => { eprintln!("Option inconnue : {}", other); std::process::exit(1); }
        }
    }
    ...
    println!("   {} × {} pixels  ({:.0} × {:.0} m = {:.2} ha)",
        chm.ncol, chm.nrow,
        chm.xmax - chm.xmin, chm.ymax - chm.ymin,
        (chm.xmax - chm.xmin) * (chm.ymax - chm.ymin) / 10000.0
    );
    println!("   Emprise : X [{:.2} → {:.2}]  Y [{:.2} → {:.2}]",
        chm.xmin, chm.xmax, chm.ymin, chm.ymax);
    println!("   CRS     : EPSG:2154 (RGF93 / Lambert-93)");

    let valid: Vec<f64> = chm.data.iter().copied()
    ...
    println!("   Hauteurs: moy={:.1}m  max={:.1}m  ({} px valides)\n",
        h_mean, h_max, valid.len());
    ...
    println!("   hmin={:.1}m  dmin={:.1}m  dprop={:.2}  sigma={:.2}  median={}",
    ...
    let dt = t0.elapsed();
    println!("   ✓ {} arbres détectés en {:.2}s\n", trees.len(), dt.as_secs_f64());
    ...
    if !trees.is_empty() {
        ...
        println!("   Hauteurs    : min={:.1}  moy={:.1}  max={:.1} m", h_min_d, h_mean_d, h_max_d);
        println!("   Surf. moy.  : {:.1} m²/arbre", s_mean);
        println!("   Couvert     : {:.1}% de la zone", s_tot / (area_ha * 10000.0) * 100.0);
        println!("   Densité     : {:.0} arbres/ha", density);

        // Histogramme par classes
        println!("\n   Classes de hauteur :");
        for &(lo, hi) in &[(0.0,10.0),(10.0,20.0),(20.0,30.0),(30.0,40.0),(40.0,50.0),(50.0,100.0)] {
    ...
    println!("\n══════════════════════════════════════════════════════════");
    println!("  ✓ Terminé — {} arbres sur {:.2} ha",
        trees.len(),
        (chm.xmax - chm.xmin) * (chm.ymax - chm.ymin) / 10000.0);
    println!("══════════════════════════════════════════════════════════");
}
